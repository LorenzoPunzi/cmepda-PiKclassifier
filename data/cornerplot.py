"""
Generates overlapping corner plots between the specified numpy arrays in order to evaluate the separation better
"""
...
def overlaid_cornerplot(array_list, array_labels, figname, variable_names):
    ...
    figure = corner.corner(array_list[0][:,:-1], bins=100, color=colors[0], labels=variable_names)

    for i in range(1, num):
        figure = corner.corner(array_list[i][:,:-1], bins=100, fig=figure, color=colors[i+7], labels=variable_names)
...
overlaid_cornerplot([arr_mc_pi,arr_mc_k], ["pions","kaons"], 'fig/overlaid_cornerplot_' +'_'.join(vars)+'.png', vars)
